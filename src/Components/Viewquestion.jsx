...
const Viewquestion = () => {
    ...
    const handlecomment = async () => {
        if (comment !== "") {
            ...
            await axios.post(`http://localhost:4000/api/comments/comments/${id}`, body).then((res) => {
                setComment("");
                setShow(false);
                getUpdatedAnswer();
    ...
    const submit = async () => {
        ...
        await axios
            .post("http://localhost:4000/api/answer/answer", body, config)
            .then(() => {
                ...
                <div style={{ width: "950px" }}>
                    ...
                    </div>
                    {
                        questionData?.comments && questionData?.comments?.map((_qd) => (
                            <div style={{ marginLeft: '100px', padding:"5px" }}>{_qd?.comment}- <span className='bg-primary' style={{ padding: "3px", borderRadius: '5px' }}>Username:</span>{new Date(_qd.created_at).toLocaleString()}</div>
                        ))
                    }
